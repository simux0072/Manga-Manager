...
class KavitaClient:
    ...
    async def _scan_folder_or_all(self, folder_path: Path) -> None:
        # Kavita returns 200 before its targeted scanner resolves the folder. If stale series
        # rows map to that folder, the asynchronous task then fails with SingleOrDefault and the
        # caller has no status to inspect or retry. Full-library scans use Kavita's safe path.
        # The synchronization handler avoids this operation entirely when its existing mapping
        # already contains every projected chapter, so this fallback is not the common path.
        del folder_path
        await self.scan_all()
