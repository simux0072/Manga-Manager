...
export const api={
  providers:(signal?:AbortSignal)=>request<{items:string[]}>('/api/v2/providers',{signal}),
  ...
  library:(q:string,sources:string[],states:string[],cursor?:string,signal?:AbortSignal)=>request<Page<Series>>(`/api/v2/library?${params({q,source:sources,state:states,cursor})}`,{signal}),
  updates:(cursor?:string,signal?:AbortSignal)=>request<Page<UpdateSeries>>(`/api/v2/updates?${params({cursor})}`,{signal}),
  ...
  matches:(cursor?:number,signal?:AbortSignal)=>request<Page<Match,number>>(`/api/v2/matches?${params({cursor})}`,{signal}),
  ...
  workloadCycle:(signal?:AbortSignal)=>request<WorkloadCycle>('/api/v2/workload-cycle',{signal}),
